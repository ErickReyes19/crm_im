...
export function Formulario({ isUpdate, initialData, usuarios }: { isUpdate: boolean; initialData?: z.infer<typeof ClienteSchema>; usuarios: Array<{ id: string; usuario: string }>; }) {
  ...
  return <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 border rounded-md p-4">
    {(["nombre","apellido","ciudad","correo","numero","direccion"] as const).map((n)=><Controller key={n} name={n} control={form.control} render={({field,fieldState})=><Field data-invalid={fieldState.invalid}><FieldLabel className="capitalize">{n}</FieldLabel><FieldContent><Input {...field} value={field.value ?? ""} /></FieldContent><FieldDescription>Ingresa {n} del cliente.</FieldDescription>{fieldState.invalid && <FieldError errors={[fieldState.error]} />}</Field>} />)}
