...
export default function ClientesListMobile({ clientes, canEdit }: { clientes: Cliente[]; canEdit: boolean }) {
  ...
  const filtered = clientes.filter((c) => `${c.nombre} ${c.apellido}`.toLowerCase().includes(searchTerm.toLowerCase()));
  return <div className="space-y-4"><Link href="/clientes/create">
    ...
    </div>{filtered.map((c) => <div key={c.id} className="p-4 border rounded-lg">
      <div className="flex justify-between"><div><p className="font-medium">{c.nombre} {c.apellido}</p>
        <p className="text-xs text-muted-foreground"></p><p className="text-xs">{c.ciudad} • {c.etiqueta}</p>
      </div>
        <div className="flex">
