...
type UserFilter<TData> = {
  enabled: boolean;
  getUserName: (row: TData) => string;
};

export function DataTable<TData, TValue>({ columns, data, userFilter }: { columns: ColumnDef<TData, TValue>[]; data: TData[]; userFilter?: UserFilter<TData> }) {
  ...
  const userOptions = React.useMemo(() => {
    ...
    return [...new Set(data.map((row) => userFilter.getUserName(row)).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  }, [data, userFilter]);
  const filteredData = React.useMemo(() => {
    if (!userFilter?.enabled || selectedUser === "todos") return data;

    return data.filter((row) => userFilter.getUserName(row) === selectedUser);
  }, [data, selectedUser, userFilter]);
