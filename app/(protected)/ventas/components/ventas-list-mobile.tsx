...
export default function VentasListMobile({ ventas, userFilter }: { ventas: VentaTableRow[]; userFilter?: UserFilter }) {
  ...
  const filtered = ventas.filter((venta) => {
    ...
    return `${venta.cliente?.nombre ?? ""} ${venta.cliente?.apellido ?? ""} ${venta.usuarioFiltro} ${venta.estado} ${venta.metodoPago ?? ""} ${venta.tipoDocumento ?? ""} ${venta.productos?.map((detalle) => detalle.producto?.nombre).join(" ") ?? ""}`.toLowerCase().includes(searchTerm.toLowerCase());
  });
