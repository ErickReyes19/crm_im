...
import { getClienteSearchText } from "@/lib/cliente-search";
...
export default function VentasListMobile({ ventas, userFilter }: { ventas: VentaTableRow[]; userFilter?: UserFilter }) {
  ...
  const filtered = ventas.filter((venta) => {
    ...
    const searchText = [
      getClienteSearchText(venta.cliente),
      venta.usuarioFiltro,
      venta.estado,
      venta.metodoPago ?? "",
      venta.tipoDocumento ?? "",
      venta.productos?.map((detalle) => detalle.producto ? getProductoLabel(detalle.producto) : "").join(" ") ?? "",
    ].join(" ").toLowerCase();
    return searchText.includes(searchTerm.toLowerCase());
  });

  return <div className="space-y-4"><Link href="/ventas/create"><Button className="w-full">Nueva venta <Plus /></Button></Link><div className="relative"><Input className="pl-10" placeholder="Buscar por nombre, teléfono o ciudad..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /></div>{userFilter?.enabled && <Select value={selectedUser} onValueChange={setSelectedUser}><SelectTrigger><SelectValue placeholder={userFilter.placeholder ?? "Clasificar por usuario"} /></SelectTrigger><SelectContent><SelectItem value="todos">Todos los usuarios</SelectItem>{userOptions.map((usuario) => <SelectItem key={usuario} value={usuario}>{usuario}</SelectItem>)}</SelectContent></Select>}<p className="text-sm text-muted-foreground">{filtered.length} venta{filtered.length === 1 ? "" : "s"}</p>{filtered.map((venta) => <div key={venta.id} className="flex justify-between rounded-lg border p-4"><div><p className="font-medium">{venta.cliente ? `${venta.cliente.nombre} ${venta.cliente.apellido}` : `Venta #${venta.id?.slice(0, 8)}`}</p><p className="text-xs">Vendedor: {venta.usuarioFiltro || "Usuario actual"}</p><EstadoVentaMobile venta={venta} /><p className="text-xs">Pago: {venta.metodoPago === "TRANSFERENCIA" ? "Transferencia" : "Efectivo"}</p><p className="text-xs">Documento: {venta.tipoDocumento === "FACTURA" ? "Factura" : "Recibo"}</p><p className="text-xs">ISV: {Number(venta.isv ?? 0).toLocaleString("es-DO", { style: "currency", currency: "HNL" })}</p><p className="text-xs">Envío: {venta.conEnvio ? Number(venta.envio ?? 0).toLocaleString("es-DO", { style: "currency", currency: "HNL" }) : "Sin envío"}</p><p className="text-xs">Productos: {venta.productos?.length ? venta.productos.map((detalle) => `${detalle.cantidad} x ${detalle.producto ? getProductoLabel(detalle.producto) : "Producto"}`).join(", ") : "Sin productos"}</p><p className="text-xs">Total: {Number(venta.total).toLocaleString("es-DO", { style: "currency", currency: "HNL" })}</p></div><Link href={`/ventas/${venta.id}/edit`}><Button variant="ghost" size="icon"><Pencil className="h-4 w-4" /></Button></Link></div>)}</div>;
}
